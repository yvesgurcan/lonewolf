...
exports.handler = (event, context, callback) => {
    if (event.password !== password) {
        ...
        dynamo.getItem(
            { TableName: 'lonewolf', Key: { gameID: event.gameID } },
            handleLoadGame
        );
        ...
        if (event.gameID !== undefined) {
            ...
            dynamo.scan(
                { TableName: 'lonewolf', AttributesToGet: ['gameID'] },
                handleGameIDs
            );
        }
    } else {
        callback(null, {
            error: true,
            message:
                "Bad request: invalid method '" + event.method + "' parameter.",
        });
    }
