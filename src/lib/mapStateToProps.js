...
export const mapStateToProps = (state, ownProps) => {
    return {
        ...
        API(request, dispatch, gameState = null) {
            ...
            let payload = {
                password:
                    state.RequestFeedback.password === undefined
                        ? undefined
                        : String(state.RequestFeedback.password),
                gameID:
                    state.RequestFeedback.gameID === undefined
                        ? ''
                        : String(state.RequestFeedback.gameID),
                ...
                if (gameState != null) {
                    APItimeout = setTimeout(function () {
                        store.dispatch({
                            ...
                            value:
                                'You must enter the password to save automatically.',
                        });
                    }, 200);
                }
            ...
            // parameters
            let url =
                'https://qdrc7541jc.execute-api.us-west-2.amazonaws.com/dev';
            let parameters = {};
            ...
            if (request === 'loadgame') {
                url +=
                    ...
                    '&password=' +
                    (encodeURIComponent(String(payload.password)) || '');
                ...
                if (window.debugApp) {
                    console.error(
                        "Unexpected request '" +
                            request +
                            "'. Unable to configure the request parameters."
                    );
                }
            ...
            fetch(url, parameters)
                ...
                .catch(function (error) {
                    ...
                    if (!navigator.onLine) {
                        return store.dispatch({
                            type: 'UPDATE_VALIDATION_REQUEST_FEEDBACK',
                            value: 'Request error: You are offline.',
                        });
                    }
                    ...
                    store.dispatch({
                        type: 'UPDATE_VALIDATION_REQUEST_FEEDBACK',
                        value: 'Request error: ' + error.message + '.',
                    });
                });
            ...
            function handleResponse(
                ...
                if (request === 'loadgame' && dispatch) {
                    ...
                    store.dispatch({
                        type: 'LOAD_GAME_FROM_API',
                        value: responseContent.gameState,
                    });
                    store.dispatch({
                        type: 'UPDATE_ACTUAL_GAME_ID_REQUEST_FEEDBACK',
                    ...
                    switch (LoreCircle) {
                        ...
                        case 'CircleFire':
                            ...
                            if (
                                magnakaiDiscipline
                                    .toLowerCase()
                                    .indexOf('magnakaiweaponmastery') > -1
                            ) {
                                requiredCount++;
                            }
                        ...
                        case 'CircleLight':
                            if (
                                magnakaiDiscipline
                                    .toLowerCase()
                                    .indexOf('magnakaianimalcontrol') > -1
                            ) {
                                requiredCount++;
                            }
                            if (
                        ...
                        case 'CircleSolaris':
                            ...
                            if (
                                magnakaiDiscipline
                                    .toLowerCase()
                                    .indexOf('magnakaiinvisibility') > -1
                            ) {
                                requiredCount++;
                            }
                            if (
                                magnakaiDiscipline
                                    .toLowerCase()
                                    .indexOf('magnakaipathsmanship') > -1
                            ) {
                                requiredCount++;
                            }
        ...
        fight(number, CombatRatio) {
            ...
            return results[
                String(Math.max(-11, Math.min(CombatRatio, 11))).replace(
                    '-',
                    'minus'
                )
            ][number];
